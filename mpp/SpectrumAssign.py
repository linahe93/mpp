...
def routing_and_spectrum(graph, paths, required_bandwidth):
    ...
    if len(sorted_paths) == 1:
        path = sorted_paths[0]
        central_frequency, start_localization = assign_spectrum(graph, path, slots_num, backup=True)
        if central_frequency is None:
            return None
        else:
        ...
        slots_for_backup = allocate_slots_for_backup_paths(slots_num)
        for path in sorted_paths:
            # allocate the slots to multiple path
            print "the slots for the backup %s" % slots_for_backup
            if slots_for_backup:
                sub_slots_num = slots_for_backup[0]
                central_frequency, start_localization = assign_spectrum(inter_graph, path, sub_slots_num, backup=True)
                print "central frequency %s" % central_frequency
                if central_frequency is None:
